...
public class NameSchemeManager extends PersistentObject implements PersistentNameSchemeManager {
			...
			class Initializer implements Runnable {
				...
				public void run() {
					try {
						...
						synchronized ($$lock) {
							proxi = ConnectionHandler.getTheConnectionHandler().theNameSchemeManagerFacade
									.getTheNameSchemeManager();
	...
	public java.util.Hashtable<String, Object> toHashtable(java.util.Hashtable<String, Object> allResults, int depth,
		...
		if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth) {
			...
			result.put("schemes",
					this.getSchemes().getVector(allResults, depth, essentialLevel, forGUI, tdObserver, false));
	...
	public NameSchemeManager provideCopy() throws PersistenceException {
		...
		result = new NameSchemeManager(this.This, this.getId());
	...
	public int getLeafInfo() throws PersistenceException {
		return (int) (0 + this.getSchemes().getLength() + this.getNames().getLength());
	}
	...
	public void createNameScheme(final String name, final String regExpPattern, final Invoker invoker)
		...
		PersistentCreateNameSchemeCommand command = model.meta.CreateNameSchemeCommand.createCreateNameSchemeCommand(
				name, regExpPattern, now, now);
