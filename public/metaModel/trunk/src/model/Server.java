...
public class Server extends PersistentObject implements PersistentServer{
    ...
    public MeasurementSearchList measurement_Path_In_DeleteEntry() 
				throws model.UserException, PersistenceException{
        		return new MeasurementSearchList(getThis().getAccountManager().
				getAccounts().
				productMultiValuedFunction(new Fnction<PersistentAccount,SearchListRoot<PersistentMeasurement>>(){
					public SearchListRoot<PersistentMeasurement> apply(PersistentAccount d) throws PersistenceException,UserException{
						return d.getEntries();
					}
				}, true));
    }
    ...
    public void deleteEntry(final PersistentAccountManager accountManager, final PersistentMeasurement measurement) 
				throws model.ConsistencyException, PersistenceException{
		getAccountManager().deleteEntry(measurement, getThis());
	}
    public void deleteEntry(final PersistentMeasurement measurement) 
				throws model.ConsistencyException, PersistenceException{
		getAccountManager().deleteEntry(measurement, getThis());
	}
