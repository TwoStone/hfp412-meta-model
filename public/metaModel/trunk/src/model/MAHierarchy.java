...
import persistence.MAHierarchyProxi;
import persistence.MAHierarchySearchList;
...
public class MAHierarchy extends PersistentObject implements PersistentMAHierarchy {
	...
	public static PersistentMAHierarchy createMAHierarchy(String name, boolean delayed$Persistence)
		...
		if (delayed$Persistence) {
			result = ConnectionHandler.getTheConnectionHandler().theMAHierarchyFacade.newDelayedMAHierarchy(name);
			result.setDelayed$Persistence(true);
		} else {
			result = ConnectionHandler.getTheConnectionHandler().theMAHierarchyFacade.newMAHierarchy(name, -1);
		}
	...
	public static PersistentMAHierarchy createMAHierarchy(String name, boolean delayed$Persistence,
			PersistentMAHierarchy This) throws PersistenceException {
		if (name == null)
			throw new PersistenceException("Null not allowed for persistent strings, since null = \"\" in Oracle!", 0);
		PersistentMAHierarchy result = null;
		if (delayed$Persistence) {
	...
	public java.util.Hashtable<String, Object> toHashtable(java.util.Hashtable<String, Object> allResults, int depth,
		...
		if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth) {
			...
			result.put(
					"associations",
					this.getAssociations().getVector(allResults, (depth > 1 ? depth : depth + 1), essentialLevel,
	...
	public static MAHierarchySearchList getMAHierarchyByName(String name) throws PersistenceException {
		return ConnectionHandler.getTheConnectionHandler().theMAHierarchyFacade.getMAHierarchyByName(name);
	}

	@Override
	public MAHierarchy provideCopy() throws PersistenceException {
		MAHierarchy result = this;
		result = new MAHierarchy(this.name, this.This, this.getId());
	...
	public void store() throws PersistenceException {
		...
		if (this.getClassId() == 142)
			ConnectionHandler.getTheConnectionHandler().theMAHierarchyFacade.newMAHierarchy(name, this.getId());
		super.store();
	...
	public void setName(String newValue) throws PersistenceException {
		if (newValue == null)
			throw new PersistenceException("Null not allowed for persistent strings, since null = \"\" in Oracle!", 0);
		if (!this.isDelayed$Persistence())
			ConnectionHandler.getTheConnectionHandler().theMAHierarchyFacade.nameSet(this.getId(), newValue);
		this.name = newValue;
	}
	...
	public int getLeafInfo() throws PersistenceException {
		return (int) (0 + this.getAssociations().getLength());
	}
	...
	@Override
	public MAssociationSearchList getAssociations() throws PersistenceException {
		MAssociationSearchList result = null;
		if (result == null)
			result = ConnectionHandler.getTheConnectionHandler().theMAssociationFacade.inverseGetHierarchies(
