...
public class FractionManager extends PersistentObject implements PersistentFractionManager {
			...
			class Initializer implements Runnable {
				...
				public void run() {
					try {
						...
						synchronized ($$lock) {
							proxi = ConnectionHandler.getTheConnectionHandler().theFractionManagerFacade
									.getTheFractionManager();
	...
	public java.util.Hashtable<String, Object> toHashtable(final java.util.Hashtable<String, Object> allResults,
		...
		if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth) {
			...
			result.put(
					"managedFractions",
					this.getManagedFractions()
							.getValues()
							.getVector(allResults, depth, essentialLevel, forGUI, tdObserver, false,
									essentialLevel == 0));
	...
	@Override
	public Fraction invertSign(final Fraction fraction) throws PersistenceException {
		final String searchString = "-" + fraction.toString();
		Fraction result = null;
		try {
			result = getThis().getFraction(searchString);
		} catch (final NotFoundException e) {
			final Fraction newF = Fraction.parse(searchString);
			try {
				getThis().addFraction(searchString, newF);
	...
	public void addFraction(final String key, final common.Fraction newFraction, final Invoker invoker)
		...
		final PersistentAddFractionCommand command = model.meta.AddFractionCommand.createAddFractionCommand(key,
				newFraction, now, now);
