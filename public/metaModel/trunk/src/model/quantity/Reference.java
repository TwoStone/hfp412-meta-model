...
public class Reference extends PersistentObject implements PersistentReference {
	...
	public static PersistentReference createReference(boolean delayed$Persistence) throws PersistenceException {
		...
		if (delayed$Persistence) {
			result = ConnectionHandler.getTheConnectionHandler().theReferenceFacade.newDelayedReference(0);
			result.setDelayed$Persistence(true);
		} else {
			result = ConnectionHandler.getTheConnectionHandler().theReferenceFacade.newReference(0, -1);
		}
	...
	public static PersistentReference createReference(boolean delayed$Persistence, PersistentReference This)
	...
	public java.util.Hashtable<String, Object> toHashtable(java.util.Hashtable<String, Object> allResults, int depth,
		...
		if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth) {
			...
			AbstractPersistentRoot ref = this.getRef();
			if (ref != null) {
				result.put("ref", ref.createProxiInformation(false));
				if (depth > 1) {
	...
	public Reference provideCopy() throws PersistenceException {
		...
		result = new Reference(this.type, this.exponent, this.ref, this.This, this.getId());
	...
	public Reference(PersistentReferenceType type, long exponent, PersistentUnit ref, PersistentReference This, long id)
	...
	public void store() throws PersistenceException {
		...
		if (this.getClassId() == 180)
			ConnectionHandler.getTheConnectionHandler().theReferenceFacade.newReference(0, this.getId());
		super.store();
	...
	public int getLeafInfo() throws PersistenceException {
		return 0 + (this.getType() == null ? 0 : 1) + (this.getRef() == null ? 0 : 1);
	}
	...
	public CompUnitSearchList inverseGetRefs() throws PersistenceException {
		...
		if (result == null)
			result = ConnectionHandler.getTheConnectionHandler().theCompUnitFacade.inverseGetRefs(this.getId(),
					this.getClassId());
		return result;
	}
