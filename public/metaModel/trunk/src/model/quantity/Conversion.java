...
public class Conversion extends PersistentObject implements PersistentConversion {
	...
	public PersistentQuantity convertInverse(final common.Fraction amount) throws PersistenceException {
		final Fraction convertedAmount = this.getThis().getMyFunction().executeInverse(amount);
	...
	public PersistentQuantity convert(final common.Fraction amount) throws PersistenceException {
		final Fraction convertedAmount = this.getThis().getMyFunction().execute(amount);
		return QuantityManager.getTheQuantityManager().createQuantity(((PersistentUnitType) this.getThis().getSource().getType()).getDefaultUnit(),
				convertedAmount);
