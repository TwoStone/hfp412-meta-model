...
public class UnitTypeManager extends PersistentObject implements PersistentUnitTypeManager {
	...
	public java.util.Hashtable<String, Object> toHashtable(final java.util.Hashtable<String, Object> allResults,
		...
		if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth) {
			...
			result.put(
					"refs",
					this.getRefs().getVector(allResults, depth, essentialLevel, forGUI, tdObserver, false,
							essentialLevel == 0));
			result.put(
					"units",
	...
	public void changeUName(final PersistentAbsUnit unit, final String name, final Invoker invoker)
			throws PersistenceException {
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentChangeUNameCommand command = model.meta.ChangeUNameCommand.createChangeUNameCommand(name, now,
				now);
		command.setUnit(unit);
		command.setInvoker(invoker);
		command.setCommandReceiver(getThis());
	...
	public void changeUTName(final PersistentAbsUnitType unitType, final String name, final Invoker invoker)
			throws PersistenceException {
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentChangeUTNameCommand command = model.meta.ChangeUTNameCommand.createChangeUTNameCommand(name,
				now, now);
		command.setUnitType(unitType);
		command.setInvoker(invoker);
		command.setCommandReceiver(getThis());
	...
	public void fetchReference(final PersistentUnit unit, final long exponent, final Invoker invoker)
			throws PersistenceException {
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentFetchReferenceCommand command = model.meta.FetchReferenceCommand.createFetchReferenceCommand(
				exponent, now, now);
		command.setUnit(unit);
		command.setInvoker(invoker);
		command.setCommandReceiver(getThis());
	...
	public void getExistingCU(final ReferenceSearchList refs, final Invoker invoker) throws PersistenceException {
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentGetExistingCUCommand command = model.meta.GetExistingCUCommand.createGetExistingCUCommand(now,
				now);
		final java.util.Iterator<PersistentReference> refsIterator = refs.iterator();
		while (refsIterator.hasNext()) {
			command.getRefs().add(refsIterator.next());
	...
	@Override
	public PersistentAbsUnit addReference(final String name, final PersistentAbsUnit unit,
			final PersistentUnit referenceUnit, final long exponent) throws model.DoubleDefinitionException,
			PersistenceException {
		final PersistentAbsUnit ret = unit
				.accept(new AbsUnitReturnExceptionVisitor<PersistentAbsUnit, DoubleDefinitionException>() {
					...
					@Override
					public PersistentAbsUnit handleCompUnit(final PersistentCompUnit compUnit)
	...
	@Override
	public void changeUName(final PersistentAbsUnit unit, final String name) throws model.DoubleDefinitionException,
			PersistenceException {
	...
	@Override
	public PersistentUnitType createUnitType(final String name) throws model.DoubleDefinitionException,
			PersistenceException {
		final AbsUnitTypeSearchList old = AbsUnitType.getAbsUnitTypeByName(name);
		if (old.iterator().hasNext()) {
			throw new DoubleDefinitionException(ExceptionConstants.DOUBLE_UNIT_TYPE_DEFINITION + name);
