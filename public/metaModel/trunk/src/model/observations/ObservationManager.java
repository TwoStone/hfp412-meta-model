...
public class ObservationManager extends PersistentObject implements PersistentObservationManager {
			...
			class Initializer implements Runnable {
				...
				public void run() {
					try {
						...
						synchronized ($$lock) {
							proxi = ConnectionHandler.getTheConnectionHandler().theObservationManagerFacade
									.getTheObservationManager();
	...
	public java.util.Hashtable<String, Object> toHashtable(final java.util.Hashtable<String, Object> allResults,
		...
		if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth) {
			...
			result.put(
					"observations",
					this.getObservations().getVector(allResults, depth, essentialLevel, forGUI, tdObserver, false,
							essentialLevel == 0));
	...
	public ObservationManager(final PersistentObservationManager This, final long id)
	...
	@Override
	public void createObservation(final String name, final PersistentMObservationType theType,
			final PersistentMObject theObsObject, final PersistentMEnumValue enumValue, final Invoker invoker)
			throws PersistenceException {
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentCreateObservationCommand command = model.meta.CreateObservationCommand
				.createCreateObservationCommand(name, now, now);
	...
	public void deleteObservation(final PersistentMObservation observation, final Invoker invoker)
			throws PersistenceException {
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentDeleteObservationCommand command = model.meta.DeleteObservationCommand
				.createDeleteObservationCommand(now, now);
	...
	@Override
	public void createObservation(final String name, final PersistentMObservationType theType,
			final PersistentMObject theObsObject, final PersistentMEnumValue enumValue) throws PersistenceException {
		MObservation.createMObservation(name, theType, enumValue, theObsObject);
	}

	@Override
	public void deleteObservation(final PersistentMObservation observation) throws model.ConsistencyException,
