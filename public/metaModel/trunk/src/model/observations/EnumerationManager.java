...
public class EnumerationManager extends PersistentObject implements PersistentEnumerationManager {
	...
	public EnumerationManager(final PersistentEnumerationManager This, final long id) throws persistence.PersistenceException {
	...
	public void createEnum(final String name, final Invoker invoker) throws PersistenceException {
		...
		final PersistentCreateEnumCommand command = model.meta.CreateEnumCommand.createCreateEnumCommand(name, now, now);
	...
	@Override
	public PersistentMEnum createEnum(final String name) throws model.DoubleDefinitionException, PersistenceException {
		if (MEnum.getMEnumByName(name).getLength() == 0) {
			final PersistentMEnum ret = MEnum.createMEnum(name);
	...
	public void deleteEnum(final PersistentMEnum type) throws model.ConsistencyException, PersistenceException {
		if (type.fetchDependentItems().getLength() == 0) {
			...
			throw new model.ConsistencyException(ExceptionConstants.CE_ENUM_DELETE_ENUMVALUE);
		}
