...
public class MEnum extends PersistentObject implements PersistentMEnum {
	...
	public static PersistentMEnum getById(final long objectId) throws PersistenceException {
		final long classId = ConnectionHandler.getTheConnectionHandler().theMEnumFacade.getClass(objectId);
		return (PersistentMEnum) PersistentProxi.createProxi(objectId, classId);
	}

	public static PersistentMEnum createMEnum(final String name) throws PersistenceException {
		return createMEnum(name, false);
	}

	public static PersistentMEnum createMEnum(final String name, final boolean delayed$Persistence)
		...
		if (delayed$Persistence) {
			result = ConnectionHandler.getTheConnectionHandler().theMEnumFacade.newDelayedMEnum(name);
			result.setDelayed$Persistence(true);
		} else {
			result = ConnectionHandler.getTheConnectionHandler().theMEnumFacade.newMEnum(name, -1);
		}
	...
	public static PersistentMEnum createMEnum(final String name, final boolean delayed$Persistence,
			final PersistentMEnum This) throws PersistenceException {
		if (name == null)
			throw new PersistenceException("Null not allowed for persistent strings, since null = \"\" in Oracle!", 0);
		PersistentMEnum result = null;
		if (delayed$Persistence) {
	...
	public static MEnumSearchList getMEnumByName(final String name) throws PersistenceException {
		return ConnectionHandler.getTheConnectionHandler().theMEnumFacade.getMEnumByName(name);
	}

	@Override
	public MEnum provideCopy() throws PersistenceException {
		MEnum result = this;
		result = new MEnum(this.name, this.This, this.myCONCMModelItem, this.getId());
	...
	public MEnum(final String name, final PersistentMEnum This, final PersistentMModelItem myCONCMModelItem,
	...
	public void store() throws PersistenceException {
		if (!this.isDelayed$Persistence())
			return;
		if (this.getClassId() == 342)
			ConnectionHandler.getTheConnectionHandler().theMEnumFacade.newMEnum(name, this.getId());
		super.store();
		...
		if (this.getMyCONCMModelItem() != null) {
			...
			ConnectionHandler.getTheConnectionHandler().theMEnumFacade.myCONCMModelItemSet(this.getId(),
					getMyCONCMModelItem());
		}
	...
	public void setName(final String newValue) throws PersistenceException {
		if (newValue == null)
			throw new PersistenceException("Null not allowed for persistent strings, since null = \"\" in Oracle!", 0);
		if (!this.isDelayed$Persistence())
			ConnectionHandler.getTheConnectionHandler().theMEnumFacade.nameSet(this.getId(), newValue);
		this.name = newValue;
	}

	protected void setThis(final PersistentMEnum newValue) throws PersistenceException {
		if (newValue == null)
	...
	public PersistentMEnum getThis() throws PersistenceException {
		if (this.This == null) {
			final PersistentMEnum result = new MEnumProxi(this.getId());
			result.getTheObject();
			return result;
		}
		return this.This;
	...
	public void initialize(final Anything This, final java.util.Hashtable<String, Object> final$$Fields)
		...
		if (this.equals(This)) {
			final PersistentCONCMModelItem myCONCMModelItem = model.CONCMModelItem.createCONCMModelItem(
					this.isDelayed$Persistence(), (PersistentMEnum) This);
	...
	@Override
	public MObservationTypeSearchList inverseGetEnumType() throws PersistenceException {
		MObservationTypeSearchList result = null;
		if (result == null)
			result = ConnectionHandler.getTheConnectionHandler().theMObservationTypeFacade.inverseGetEnumType(
					this.getId(), this.getClassId());
		return result;
	}

	@Override
	public MEnumValueSearchList inverseGetTheType() throws PersistenceException {
		MEnumValueSearchList result = null;
		if (result == null)
			result = ConnectionHandler.getTheConnectionHandler().theMEnumValueFacade.inverseGetTheType(this.getId(),
					this.getClassId());
		return result;
	}
