...
public class OperationManager extends PersistentObject implements PersistentOperationManager {
	...
	public java.util.Hashtable<String, Object> toHashtable(final java.util.Hashtable<String, Object> allResults, final int depth,
		...
		if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth) {
			...
			result.put("operations", this.getOperations()
					.getVector(allResults, depth, essentialLevel, forGUI, tdObserver, false, essentialLevel == 0));
			result.put(
					"staticOperations",
					this.getStaticOperations(tdObserver).getVector(allResults, (depth > 1 ? depth : depth + 1), essentialLevel, forGUI, tdObserver,
							false, essentialLevel == 0));
			result.put(
					"constants",
					this.getConstants(tdObserver).getVector(allResults, (depth > 1 ? depth : depth + 1), essentialLevel, forGUI, tdObserver, false,
							essentialLevel == 0));
			result.put("formalParameters",
					this.getFormalParameters().getVector(allResults, depth, essentialLevel, forGUI, tdObserver, false, essentialLevel == 0));
	...
	public OperationManager(final PersistentOperationManager This, final long id) throws persistence.PersistenceException {
	...
	@Override
	public void addFp(final PersistentOperation op, final PersistentFormalParameter fp, final Invoker invoker) throws PersistenceException {
	...
	@Override
	public void addMultipleFp(final PersistentOperation op, final FormalParameterSearchList fp, final Invoker invoker) throws PersistenceException {
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentAddMultipleFpCommand command = model.meta.AddMultipleFpCommand.createAddMultipleFpCommand(now, now);
		command.setOp(op);
	...
	@Override
	public void createConstant(final String name, final PersistentMType target, final Invoker invoker) throws PersistenceException {
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentCreateConstantCommand command = model.meta.CreateConstantCommand.createCreateConstantCommand(name, now, now);
	...
	@Override
	public void createFp(final String name, final PersistentMType ofType, final Invoker invoker) throws PersistenceException {
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
	...
	public void createOperation(final PersistentMType source, final PersistentMType target, final String name, final FormalParameterSearchList fp,
		...
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentCreateOperationCommand command = model.meta.CreateOperationCommand.createCreateOperationCommand(name, now, now);
	...
	public void createStaticOp(final String name, final PersistentMType target, final FormalParameterSearchList fp, final Invoker invoker)
		...
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentCreateStaticOpCommand command = model.meta.CreateStaticOpCommand.createCreateStaticOpCommand(name, now, now);
		command.setTarget(target);
		final java.util.Iterator<PersistentFormalParameter> fpIterator = fp.iterator();
		while (fpIterator.hasNext()) {
			command.getFp().add(fpIterator.next());
	...
	public void createVoidOperation(final PersistentMType source, final String name, final FormalParameterSearchList fp, final Invoker invoker)
			throws PersistenceException {
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentCreateVoidOperationCommand command = model.meta.CreateVoidOperationCommand.createCreateVoidOperationCommand(name, now, now);
		command.setSource(source);
		final java.util.Iterator<PersistentFormalParameter> fpIterator = fp.iterator();
		while (fpIterator.hasNext()) {
			command.getFp().add(fpIterator.next());
	...
	@Override
	public void removeFpFromOp(final PersistentOperation op, final PersistentFormalParameter fp, final Invoker invoker) throws PersistenceException {
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentRemoveFpFromOpCommand command = model.meta.RemoveFpFromOpCommand.createRemoveFpFromOpCommand(now, now);
		command.setOp(op);
		command.setFp(fp);
		command.setInvoker(invoker);
		command.setCommandReceiver(getThis());
	...
	public void removeOperation(final PersistentOperation op, final Invoker invoker) throws PersistenceException {
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentRemoveOperationCommand command = model.meta.RemoveOperationCommand.createRemoveOperationCommand(now, now);
		command.setOp(op);
		command.setInvoker(invoker);
		command.setCommandReceiver(getThis());
		model.meta.CommandCoordinator.getTheCommandCoordinator().coordinate(command);
	...
	@Override
	public void addFp(final PersistentOperation op, final PersistentFormalParameter fp) throws model.ConsistencyException, PersistenceException {
	...
	@Override
	public void addMultipleFp(final PersistentOperation op, final FormalParameterSearchList fp) throws model.ConsistencyException,
	...
	@Override
	public PersistentOperation createConstant(final String name, final PersistentMType target) throws model.DoubleDefinitionException,
			model.ConsistencyException, PersistenceException {
		final PersistentMEmptyTypeDisjunction theMEmptyTypeDisjunction = MEmptyTypeDisjunction.getTheMEmptyTypeDisjunction();
		return getThis().createOperation(theMEmptyTypeDisjunction, target, name, new FormalParameterSearchList());
	}

	@Override
	public PersistentFormalParameter createFp(final String name, final PersistentMType ofType) throws model.DoubleDefinitionException,
			model.ConsistencyException, PersistenceException {
	...
	@Override
	public PersistentOperation createOperation(final PersistentMType source, final PersistentMType target, final String name,
			final FormalParameterSearchList fp) throws model.DoubleDefinitionException, model.ConsistencyException, PersistenceException {
		// DDE Pruefen
	...
	@Override
	public void removeFpFromOp(final PersistentOperation op, final PersistentFormalParameter fp) throws model.NotAvailableException,
			model.ConsistencyException, PersistenceException {
		...
		if (!deleted) {
			throw new NotAvailableException(ExceptionConstants.NA_FP);
		}
	}
	...
	private void checkOperationDoubleDefinition(final String name) throws DoubleDefinitionException, PersistenceException {
		...
		while (iterator.hasNext()) {
			iterator.next().accept(new AbsOperationReturnExceptionVisitor<AbsOperation, DoubleDefinitionException>() {
				...
				@Override
				public AbsOperation handleOperation(final PersistentOperation operation) throws PersistenceException, DoubleDefinitionException {
					throw new DoubleDefinitionException(ExceptionConstants.DDE_OP);
				}

				@Override
				public AbsOperation handleAssociation(final PersistentAssociation association) throws PersistenceException, DoubleDefinitionException {
					return null;
				}
			});
		}
