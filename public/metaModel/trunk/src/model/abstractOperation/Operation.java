...
public class Operation extends model.abstractOperation.AbsOperation implements PersistentOperation {
	...
	public static PersistentOperation createOperation(final String name, final PersistentMType source, final PersistentMType target)
	...
	public static PersistentOperation createOperation(final String name, final PersistentMType source, final PersistentMType target,
			final boolean delayed$Persistence, final PersistentOperation This) throws PersistenceException {
		PersistentOperation result = null;
		if (delayed$Persistence) {
			result = ConnectionHandler.getTheConnectionHandler().theOperationFacade.newDelayedOperation(name);
			result.setDelayed$Persistence(true);
	...
	public Operation provideCopy() throws PersistenceException {
		...
		result = new Operation(this.name, this.source, this.target, this.This, this.myCONCMModelItem, this.getId());
	...
	public Operation(final String name, final PersistentMType source, final PersistentMType target, final PersistentAbsOperation This,
	...
	public MessageSearchList inverseGetType() throws PersistenceException {
		...
		if (result == null)
			result = ConnectionHandler.getTheConnectionHandler().theMessageFacade.inverseGetType(this.getId(), this.getClassId());
		return result;
	}
