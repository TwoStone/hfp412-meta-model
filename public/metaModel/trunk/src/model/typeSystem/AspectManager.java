...
public class AspectManager extends PersistentObject implements PersistentAspectManager {
	...
	private boolean existsAspectWithName(final String name) throws PersistenceException {
		final Iterator<PersistentMAspect> iterator = MAspect.getMAspectByName(name).iterator();
		while (iterator.hasNext()) {
			final PersistentMAspect next = iterator.next();
			System.out.println("Asp:" + next.isDltd());
			System.out.println("AspCMI: " + next.getMyCONCMModelItem().isDltd());
			if (!next.isDltd()) {
