...
public class MObject extends PersistentObject implements PersistentMObject {
	...
	@Override
	public MObjectSearchList determinePossibleLinkTargets() throws PersistenceException {
		// TODO: Laesst sich ggf. etwas optimieren wenn man die beiden Iterationen nicht von einander trennt aber dann
		// versteht es niemand mehr...
		final MObjectSearchList possibleLinkTargets = new MObjectSearchList();

		AssociationSearchList associations = new AssociationSearchList();

		// 1. Alle Assoziationen ermitteln, in denen einer meiner Typen als source enthalten ist
		Iterator<PersistentMAtomicType> myTypesIterator = getThis().getTypes().iterator();

		PersistentMType currentType = null;
		while (myTypesIterator.hasNext()) {
			currentType = myTypesIterator.next();

			associations.add((PersistentAssociation) currentType.inverseGetSource().findAll(
					new Predcate<PersistentAbsOperation>() {

						@Override
						public boolean test(PersistentAbsOperation argument) throws PersistenceException {
							return argument.accept(new AbsOperationReturnVisitor<MBoolean>() {

								@Override
								public MBoolean handleOperation(PersistentOperation operation)
										throws PersistenceException {
									return (MBoolean) MFalse.getTheMFalse();
								}

								@Override
								public MBoolean handleAssociation(PersistentAssociation association)
										throws PersistenceException {
									return (MBoolean) MTrue.getTheMTrue();
								}
							}).toBoolean();
						}
					}));
		...
		// 2. Durch die gefundenen Assoziationen iterieren und die Targets unter umstaenden
		// in die Liste der possibleLinkTargets packen
		associations.applyToAll(new Procdure<PersistentAssociation>() {

			@Override
			public void doItTo(PersistentAssociation argument) throws PersistenceException {
				possibleLinkTargets.add(getPossibleLinkTargets((argument.getTarget())));
			}
		});
