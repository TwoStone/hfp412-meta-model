...
public class MAtomicType extends model.typeSystem.MType implements PersistentMAtomicType {
	...
	public static PersistentMAtomicType createMAtomicType(final String name, final PersistentMBoolean singletonType,
			final PersistentMBoolean abstractType, final PersistentMAspect aspect) throws PersistenceException {
		return createMAtomicType(name, singletonType, abstractType, aspect, false);
	}

	public static PersistentMAtomicType createMAtomicType(final String name, final PersistentMBoolean singletonType,
			final PersistentMBoolean abstractType, final PersistentMAspect aspect, final boolean delayed$Persistence)
	...
	public static PersistentMAtomicType createMAtomicType(final String name, final PersistentMBoolean singletonType,
			final PersistentMBoolean abstractType, final PersistentMAspect aspect, final boolean delayed$Persistence,
			final PersistentMAtomicType This) throws PersistenceException {
		if (name == null)
			throw new PersistenceException("Null not allowed for persistent strings, since null = \"\" in Oracle!", 0);
		PersistentMAtomicType result = null;
		if (delayed$Persistence) {
	...
	public java.util.Hashtable<String, Object> toHashtable(final java.util.Hashtable<String, Object> allResults,
		...
		if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth) {
			...
			result.put(
					"nEATCContainingMe",
					this.getNEATCContainingMe().getVector(allResults, (depth > 1 ? depth : depth + 1), essentialLevel,
			...
			result.put(
					"possibleNames",
					this.getPossibleNames().getVector(allResults, (depth > 1 ? depth : depth + 1), essentialLevel,
							forGUI, tdObserver, false, essentialLevel == 0));
	...
	public MAtomicType provideCopy() throws PersistenceException {
		...
		result = new MAtomicType(this.This, this.myCONCMModelItem, this.name, this.singletonType, this.abstractType,
				this.aspect, this.superType, this.getId());
	...
	public MAtomicType(final PersistentMType This, final PersistentMModelItem myCONCMModelItem, final String name,
			...
			final PersistentMAspect aspect, final PersistentMAtomicType superType, final long id)
	...
	@Override
	public void setSuperType(final PersistentMAtomicType newValue) throws PersistenceException, model.CycleException {
		if (newValue == null)
			throw new PersistenceException("Null values not allowed!", 0);
	...
	@Override
	public <E extends UserException> void accept(final MAtomicTypeHierarchyHIERARCHYExceptionVisitor<E> visitor)
			throws PersistenceException, E {
		visitor.handleMAtomicType(this);
	}
	...
	public int getLeafInfo() throws PersistenceException {
		...
		if (this.getMTCContainingMe().getLength() > 0)
			return 1;
		if (this.getNEATCContainingMe().getLength() > 0)
			return 1;
	...
	public NameSearchList getPossibleNames() throws PersistenceException {
		...
		if (result == null)
			result = ConnectionHandler.getTheConnectionHandler().theNameFacade.inverseGetFromType(this.getId(),
					this.getClassId());
		return result;
	}
	...
	public void initialize(final Anything This, final java.util.Hashtable<String, Object> final$$Fields)
		...
		if (this.equals(This)) {
			final PersistentCONCMModelItem myCONCMModelItem = model.CONCMModelItem.createCONCMModelItem(
					this.isDelayed$Persistence(), (PersistentMAtomicType) This);
	...
	public MObjectSearchList inverseGetTypes() throws PersistenceException {
		...
		if (result == null)
			result = ConnectionHandler.getTheConnectionHandler().theMObjectFacade.inverseGetTypes(this.getId(),
					this.getClassId());
		return result;
	}
