...
public class MEmptyProductType extends model.typeSystem.MAbstractProductType implements PersistentMEmptyProductType {
	...
	public <T> T strategyMComplexTypeHierarchy(final T parameter,
		...
		T result$$containedTypes$$MEmptyProductType = strategy.initialize$$MEmptyProductType$$containedTypes(getThis(),
				parameter);
		...
		while (iterator$$.hasNext()) {
			...
			T current$$ = current$$Field.strategyMComplexTypeHierarchy(result$$containedTypes$$MEmptyProductType,
					strategy);
			result$$containedTypes$$MEmptyProductType = strategy.consolidate$$MEmptyProductType$$containedTypes(
					getThis(), result$$containedTypes$$MEmptyProductType, current$$);
		}
