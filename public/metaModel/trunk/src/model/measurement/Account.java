...
public class Account extends model.measurement.QuantifObject implements PersistentAccount {
	...
	public static PersistentAccount createAccount(final PersistentMObject object, final PersistentMAccountType type)
	...
	public static PersistentAccount createAccount(final PersistentMObject object, final PersistentMAccountType type,
		...
		if (delayed$Persistence) {
			result = ConnectionHandler.getTheConnectionHandler().theAccountFacade.newDelayedAccount();
			result.setDelayed$Persistence(true);
		} else {
			result = ConnectionHandler.getTheConnectionHandler().theAccountFacade.newAccount(-1);
		}
	...
	public java.util.Hashtable<String, Object> toHashtable(final java.util.Hashtable<String, Object> allResults,
		...
		if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth) {
			...
			result.put(
					"subAccounts",
					this.getSubAccounts().getVector(allResults, depth, essentialLevel, forGUI, tdObserver, false,
							essentialLevel == 0));
			result.put(
					"entries",
					this.getEntries().getVector(allResults, depth, essentialLevel, forGUI, tdObserver, false,
							essentialLevel == 0));
	...
	public Account provideCopy() throws PersistenceException {
		Account result = this;
		result = new Account(this.object, this.This, this.type, this.getId());
	...
	public Account(final PersistentMObject object, final PersistentQuantifObject This,
			final PersistentMAccountType type, final long id) throws persistence.PersistenceException {
		/* Shall not be used by clients for object construction! Use static create operation instead! */
		super(object, This, id);
		this.type = type;
	...
	public void store() throws PersistenceException {
		...
		if (this.getClassId() == 183)
			ConnectionHandler.getTheConnectionHandler().theAccountFacade.newAccount(this.getId());
		super.store();
	...
	@Override
	public <E extends UserException> void accept(final QuantifObjectExceptionVisitor<E> visitor)
			throws PersistenceException, E {
		visitor.handleAccount(this);
	}

	@Override
	public <R, E extends UserException> R accept(final QuantifObjectReturnExceptionVisitor<R, E> visitor)
			throws PersistenceException, E {
		return visitor.handleAccount(this);
	}
	...
	@Override
	public <E extends UserException> void accept(final AccountHierarchyHIERARCHYExceptionVisitor<E> visitor)
			throws PersistenceException, E {
		visitor.handleAccount(this);
	}

	@Override
	public <R, E extends UserException> R accept(final AccountHierarchyHIERARCHYReturnExceptionVisitor<R, E> visitor)
			throws PersistenceException, E {
		return visitor.handleAccount(this);
	}
	...
	public void addSubAccount(final PersistentAccount account, final Invoker invoker) throws PersistenceException {
		final java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		final PersistentAddSubAccountCommand command = model.meta.AddSubAccountCommand.createAddSubAccountCommand(now,
				now);
	...
	public <T> T strategyAccountHierarchy(final T parameter, final AccountHierarchyHIERARCHYStrategy<T> strategy)
		...
		while (iterator$$.hasNext()) {
			...
			result$$subAccounts$$Account = strategy.consolidate$$Account$$subAccounts(getThis(),
					result$$subAccounts$$Account, current$$);
		}
		return strategy.finalize$$Account(getThis(), parameter, result$$subAccounts$$Account);
	}

	@Override
	public void addEntry(final PersistentMeasurement measurement) throws model.ConsistencyException,
	...
	@Override
	public void addSubAccount(final PersistentAccount account) throws model.ConsistencyException, model.CycleException,
			PersistenceException {
	...
	@Override
	public PersistentAbsQuantity aggregate(final AggregationStrategy strategy) throws model.NotComputableException,
			PersistenceException {
