...
public class AccountManager extends PersistentObject implements PersistentAccountManager {
			...
			class Initializer implements Runnable {
				...
				public void run() {
					try {
						...
						synchronized ($$lock) {
							proxi = ConnectionHandler.getTheConnectionHandler().theAccountManagerFacade
									.getTheAccountManager();
	...
	public java.util.Hashtable<String, Object> toHashtable(java.util.Hashtable<String, Object> allResults, int depth,
		...
		if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth) {
			...
			result.put("accounts",
					this.getAccounts().getVector(allResults, depth, essentialLevel, forGUI, tdObserver, false));
	...
	public int getLeafInfo() throws PersistenceException {
		return (int) (0 + this.getAccounts().getLength());
	}
	...
	@Override
	public void createAccount(final String name, final PersistentMAccountType type,
			final PersistentInstanceObject object) throws PersistenceException {
		this.getThis().getAccounts().add(Account.createAccount(object, type));
	}

	@Override
	public void createAccount(final String name, final PersistentMAccountType type,
			final PersistentInstanceObject object, final Invoker invoker) throws PersistenceException {
		java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		PersistentCreateAccountCommand command = model.meta.CreateAccountCommand.createCreateAccountCommand(name, now,
				now);
