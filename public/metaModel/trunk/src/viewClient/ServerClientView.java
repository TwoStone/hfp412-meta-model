...
public class ServerClientView extends JPanel implements ExceptionAndEventHandler {
		...
		if (selected != null) {
			...
			if (selected instanceof NameSchemeManagerView) {
				...
				item.addActionListener(new java.awt.event.ActionListener() {
					@Override
					public void actionPerformed(java.awt.event.ActionEvent e) {
						ServerCreateNameSchemeNameSchemeManagerStringStringMssgWizard wizard = new ServerCreateNameSchemeNameSchemeManagerStringStringMssgWizard(
			...
			if (selected instanceof CompUnitView) {
				...
				item.addActionListener(new java.awt.event.ActionListener() {
					@Override
					public void actionPerformed(java.awt.event.ActionEvent e) {
						ServerAddReferenceCompUnitUnitIntegerMssgWizard wizard = new ServerAddReferenceCompUnitUnitIntegerMssgWizard(
								"addReference");
			...
			if (selected instanceof CompUnitTypeView) {
				...
				item.addActionListener(new java.awt.event.ActionListener() {
					@Override
					public void actionPerformed(java.awt.event.ActionEvent e) {
						if (javax.swing.JOptionPane.showConfirmDialog(getNavigationPanel(), "Publish"
								+ Wizard.ConfirmQuestionMark, "Best�tigen", javax.swing.JOptionPane.OK_CANCEL_OPTION,
								javax.swing.JOptionPane.QUESTION_MESSAGE, null) == javax.swing.JOptionPane.YES_OPTION) {
							try {
				...
				item.addActionListener(new java.awt.event.ActionListener() {
					@Override
					public void actionPerformed(java.awt.event.ActionEvent e) {
						ServerAddReferenceTypeCompUnitTypeUnitTypeIntegerMssgWizard wizard = new ServerAddReferenceTypeCompUnitTypeUnitTypeIntegerMssgWizard(
								"addReferenceType");
	...
	class ServerCreateFpOperationManagerStringMTypeMssgWizard extends Wizard {
		...
		protected void addParameters() {
			getParametersPanel().add(new StringSelectionPanel("name", this));
			getParametersPanel().add(
					new ObjectSelectionPanel("ofType", "view.MTypeView", (ViewRoot) getConnection().getServerView(),
							this));
		}
	...
	class ServerAddReferenceCompUnitUnitIntegerMssgWizard extends Wizard {
		...
		protected void addParameters() {
			try {
				getParametersPanel().add(
						new ObjectSelectionPanel("unit", "view.UnitView", new ListRoot(getConnection().getServerView()
								.getUnitTypeManager().getUnits()), this));
			} catch (ModelException me) {
				;
				handleException(me);
				setVisible(false);
		...
		protected void perform() {
			try {
				getConnection().addReferenceType(firstArgument,
						(UnitTypeView) ((ObjectSelectionPanel) getParametersPanel().getComponent(0)).getResult(),
						((IntegerSelectionPanel) getParametersPanel().getComponent(1)).getResult().longValue());
				getConnection().setEagerRefresh();
				setVisible(false);
				dispose();
