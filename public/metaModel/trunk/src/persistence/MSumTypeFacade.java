...
public class MSumTypeFacade{
    ...
    public MSumTypeProxi newMSumType(long createMinusStorePlus) throws PersistenceException {
        ...
        try{
            ...
            return (MSumTypeProxi)PersistentProxi.createProxi(id, 144);
    ...
    public MSumTypeProxi newDelayedMSumType() throws PersistenceException {
        OracleCallableStatement callable;
        try{
            callable = (OracleCallableStatement)this.con.prepareCall("Begin ? := " + this.schemaName + ".MSmTpFacade.newDelayedMSmTp(); end;");
            callable.registerOutParameter(1, OracleTypes.NUMBER);
            callable.execute();
            long id = callable.getLong(1);
            callable.close();
