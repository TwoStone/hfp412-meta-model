...
public class OperationManager extends PersistentObject implements PersistentOperationManager{
    ...
    java.util.Hashtable<String,Object> result = null;
        if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth){
            ...
            result.put("operations", this.getOperations().getVector(allResults, depth, essentialLevel, forGUI, tdObserver, false));
            result.put("staticOperations", this.getStaticOperations(tdObserver).getVector(allResults, (depth > 1 ? depth : depth + 1), essentialLevel, forGUI, tdObserver, false));
            result.put("constants", this.getConstants(tdObserver).getVector(allResults, (depth > 1 ? depth : depth + 1), essentialLevel, forGUI, tdObserver, false));
            result.put("formalParameters", this.getFormalParameters().getVector(allResults, depth, essentialLevel, forGUI, tdObserver, false));
    ...
    public int getLeafInfo() throws PersistenceException{
        return (int) (0 
            + this.getOperations().getLength()
            + this.getStaticOperations().getLength()
            + this.getConstants().getLength()
            + this.getFormalParameters().getLength());
    }
