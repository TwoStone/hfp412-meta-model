...
public class TypeManager extends PersistentObject implements PersistentTypeManager {
	...
	@Override
	public PersistentMAbstractSumType createSumType(final MTypeSearchList addends) throws model.ConsistencyException,
		...
		if (filteredAtomicProducts.size() == addends.getLength()) {
			PersistentMDisjuncitveNF resultMDisjuncitveNF = MDisjuncitveNF.createMDisjuncitveNF(true);
			Iterator<PersistentMAtomicTypeProduct> iterator = filteredAtomicProducts.iterator();
			while (iterator.hasNext()) {
				try {
					resultMDisjuncitveNF.getAddends().add(iterator.next());
				} catch (CycleException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			}
			result = resultMDisjuncitveNF;
		} else {
	...
	@Override
	public void createSumType(final MTypeSearchList addends, final Invoker invoker) throws PersistenceException {
		java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
	...
	@Override
	public void createProductType(final MTypeSearchList factors, final Invoker invoker) throws PersistenceException {
		java.sql.Date now = new java.sql.Date(new java.util.Date().getTime());
		PersistentCreateProductTypeCommand command = model.meta.CreateProductTypeCommand
				.createCreateProductTypeCommand(now, now);
		...
		list.applyToAll(new Procdure<PersistentMType>() {
			...
			public void doItTo(PersistentMType argument) throws PersistenceException {
				argument.accept(new MTypeVisitor() {
					...
					@Override
					public void handleMDisjuncitveNF(PersistentMDisjuncitveNF mDisjuncitveNF)
							throws PersistenceException {
					}
