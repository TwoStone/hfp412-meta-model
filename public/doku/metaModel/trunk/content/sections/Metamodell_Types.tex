...
mit gelöscht. Die \emph{changeSingleton}-Operation verwaltet ebenfalls \emph{SingletonObject}s. In den den entsprechenden Operationen im 
...
\begin{description}
\item[createTypeConjunction] legt einen normalisierten \qq{UND-Typen} an, falls noch kein strukturell äquivalenter Typ verwaltet wird. Andernfalls wird dieser zurückgegen.
\item[createTypeDisjunction] legt einen normalisierten \qq{ODER-Typen} an, falls noch kein strukturell äquivalenter Typ verwaltet wird. Andernfalls wird dieser zurückgegen.
...
\textbf{Persistenz} \newline
Komplexe Typen werden grundsätzlich transient erstellt und erst wenn vom im \term{TypeManager} verwaltet werden persistiert. 
