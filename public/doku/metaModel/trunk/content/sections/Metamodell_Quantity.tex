...
Auf der Typebene gibt es die beiden atomaren UnitTypes "`Strecke"' und "`Zeit"', außerdem zwei CompoundUnitTypes "`Geschwindigkeit"' und "`Beschleunigung"'. Geschwindigkeit hat zwei Referenzen (RefTypes), eine mit dem Exponent 1 auf Strecke und eine mit dem Exponenten -1 auf Zeit. Beschleunigung hat ebenfalls eine Referenz mit dem Exponenten 1 auf Strecke. Hierbei wird das selbe RefType-Objekt verwendet, welches auch Geschwindigkeit nutzt. Außerdem besitzt Beschleunigung noch eine Referenz auf Zeit. Da diese jedoch den Exponenten -2 hat, wird hierbei ein anderes RefType-Objekt verwendet als bei Geschwindigkeit.

Auf der Exemplarebene gibt es die atomaren Units "`km"', typisiert in "`Strecke"', und "`h" und "'s"`, beide typisiert in "'Zeit"'. Außerdem gibt es die CompoundUnits "`km/h"', welche als Einheitentyp "`Geschwindigkeit"' hat, und "`km/(s*h)"', welche zum Einheitentyp "`Beschleunigung"' gehört. Auf km gibt es eine Referenz mit Exponent 1, welche sowohl von km/h als auch von km/(s*h) verwendet wird. Die Referenz mit Exponent -1 auf h wird ebenfalls von km/h und km/(s*h) benutzt. Außerdem geht von km/(s*h) aus noch eine Referenz mit Exponenten -1 auf s. Somit gibt es auf der Exemplarebene von der Beschleunigungs-Einheit zwei Referenzen auf Zeit-Einheiten. Summiert man jedoch ihre Exponenten, erhält man den auf der Typebene geforderten Exponenten -2.
