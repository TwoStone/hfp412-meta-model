...
Sowohl Operationen als auch Assoziationen haben genau ein Quell- und Zieltyp wobei diese in den unterschiedlichen Kontexten (Operation oder Assoziation) semantisch unterschiedlich interpretiert werden. Im Folgenden werden sowohl die Meta- als auch Exemplarebene erläutert und alle Operationen beschrieben, die es dem Anwender erlauben Operationen und Assoziationen sowie Messages\footnote{Wie im Folgenden erläutert wird, sind Messages nicht Bestandteil der ersten Implementierungsstufe. Sie wurden nur aus Gründen der Vollständigkeit mit aufgeführt.} und Links zu verwalten.  

\img[width=\relWidth{0.7},viewport=150 520 790 1330, clip]{klassendiagramm/Klassendiagramm.pdf}{Klassendiagramm - Ausschnitt Operation}{img_operation}
\subsubsection{Operationen}

Die Klasse Operation repräsentiert die Abbildung von Operationen in dem erarbeiteten Modell.
Wie im Modell (siehe \refImg{img_operation}) ersichtlich, hat die Oberklasse AbstractOperation zwei  Assoziationen zu MType. 
...
Im Folgenden werden diese Operationen kurz beschrieben, wobei sowohl \textbf{Parameter} als auch \emph{Exceptions} durch entsprechende Formatierungen hervorgehoben werden.
