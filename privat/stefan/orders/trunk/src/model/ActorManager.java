...
public class ActorManager extends PersistentObject implements PersistentActorManager{
    ...
    public void createActor(final String name) 
        ...
        ActorSearchList old = Actor.getActorByName(name);
        if (old.iterator().hasNext()) throw new DoubleDefinition(name + " existiert bereits");
        getThis().getActors().add(Actor.createActor(name));
