...
public class MetaAssociationFacade{
    ...
    public MetaAssociation getMetaAssociation(long MetaAssociationId) throws PersistenceException{
        try{
            ...
            PersistentAbstractMetaType fromType = null;
            if (obj.getLong(5) != 0)
                fromType = (PersistentAbstractMetaType)PersistentProxi.createProxi(obj.getLong(5), obj.getLong(6));
            PersistentAbstractMetaType toType = null;
            if (obj.getLong(7) != 0)
                toType = (PersistentAbstractMetaType)PersistentProxi.createProxi(obj.getLong(7), obj.getLong(8));
    ...
    public void fromTypeSet(long MetaAssociationId, PersistentAbstractMetaType fromTypeVal) throws PersistenceException {
        try{
            CallableStatement callable;
            callable = this.con.prepareCall("Begin " + this.schemaName + ".MtAssctnFacade.frmTpSet(?, ?, ?); end;");
            callable.setLong(1, MetaAssociationId);
            callable.setLong(2, fromTypeVal.getId());
            callable.setLong(3, fromTypeVal.getClassId());
            callable.execute();
            callable.close();
        }catch(SQLException se) {
            throw new PersistenceException(se.getMessage(), se.getErrorCode());
    ...
    public void toTypeSet(long MetaAssociationId, PersistentAbstractMetaType toTypeVal) throws PersistenceException {
        try{
            CallableStatement callable;
            callable = this.con.prepareCall("Begin " + this.schemaName + ".MtAssctnFacade.tTpSet(?, ?, ?); end;");
            callable.setLong(1, MetaAssociationId);
            callable.setLong(2, toTypeVal.getId());
            callable.setLong(3, toTypeVal.getClassId());
            callable.execute();
            callable.close();
        }catch(SQLException se) {
            throw new PersistenceException(se.getMessage(), se.getErrorCode());
