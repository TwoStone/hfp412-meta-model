...
public class ServerProxi extends ViewProxi implements ServerView{
    ...
    public ServerView getRemoteObject(java.util.Hashtable<String,Object> resultTable, ExceptionAndEventHandler connectionKey) throws ModelException{
        ViewProxi actors = null;
        String actors$String = (String)resultTable.get("actors");
        if (actors$String != null) {
            common.ProxiInformation actors$Info = common.RPCConstantsAndServices.createProxiInformation(actors$String);
            actors = ViewProxi.createProxi(actors$Info,connectionKey);
            actors.setToString(actors$Info.getToString());
        }
        java.util.Vector<String> errors_string = (java.util.Vector<String>)resultTable.get("errors");
        java.util.Vector<ErrorDisplayView> errors = ViewProxi.getProxiVector(errors_string, connectionKey);
        String user = (String)resultTable.get("user");
        ServerView result$$ = new Server((ActorManagerView)actors,errors,(String)user, this.getId(), this.getClassId());
    ...
    public ViewObjectInTree getChild(int originalIndex) throws ModelException {
        ...
        if(this.getActors() != null && index < this.getActors().getTheObject().getChildCount())
            return this.getActors().getTheObject().getChild(index);
        if(this.getActors() != null) index = index - this.getActors().getTheObject().getChildCount();
        return null;
    }
    public int getChildCount() throws ModelException {
        return 0 
            + (this.getActors() == null ? 0 : this.getActors().getTheObject().getChildCount());
    }
    public boolean isLeaf() throws ModelException {
        if (this.object == null) return this.getLeafInfo() == 0;
        return true 
            && (this.getActors() == null ? true : this.getActors().getTheObject().isLeaf());
    }
    public int getIndexOfChild(Object child) throws ModelException {
        int result = 0;
        if(this.getActors() != null && this.getActors().equals(child)) return result;
        if(this.getActors() != null) result = result + 1;
        return -1;
    }
    
    public ActorManagerView getActors() throws ModelException {
        return ((Server)this.getTheObject()).getActors();
    }
