...
public  class RemoteServer extends RemoteServerMaster {
    ...
    public synchronized java.util.Hashtable<?,?> addRole(String akteurProxiString, String roleName){
        try {
            PersistentActor akteur = (PersistentActor)PersistentProxi.createProxi(common.RPCConstantsAndServices.createProxiInformation(akteurProxiString));
            ((PersistentServer)this.server).addRole(akteur, roleName);
            return createOKResult();
        }catch(PersistenceException pe){
            return createExceptionResult(pe);
        }
